...
static void test_wal_roundtrip(void) {
    ...
    Wal *wal = NULL;
    assert(wal_open(path, false, &wal) == LSM_OK);
    assert(wal != NULL);

    assert(wal_append_put(wal, lsm_slice_str("alpha"), lsm_slice_str("one"), 1) == LSM_OK);
    assert(wal_append_put(wal, lsm_slice_str("beta"), lsm_slice_str("two"), 2) == LSM_OK);
    assert(wal_append_delete(wal, lsm_slice_str("alpha"), 3) == LSM_OK);
    assert(wal_sync(wal) == LSM_OK);
    wal_close(wal);

    assert(wal_open(path, false, &wal) == LSM_OK);
    ReplayLog log;
    memset(&log, 0, sizeof(log));
    assert(wal_recover(wal, capture, &log) == LSM_OK);
    wal_close(wal);

    assert(log.count == 3);
    assert(log.ops[0] == 1);
    assert(strcmp(log.keys[0], "alpha") == 0);
    assert(strcmp(log.values[0], "one") == 0);
    assert(log.seqs[0] == 1);

    assert(log.ops[1] == 1);
    assert(strcmp(log.keys[1], "beta") == 0);
    assert(strcmp(log.values[1], "two") == 0);
    assert(log.seqs[1] == 2);

    assert(log.ops[2] == 2);
    assert(strcmp(log.keys[2], "alpha") == 0);
    assert(log.seqs[2] == 3);
...
static void test_wal_sync_flag(void) {
    ...
    Wal *wal = NULL;
    assert(wal_open(path, true, &wal) == LSM_OK);
    assert(wal_append_put(wal, lsm_slice_str("k"), lsm_slice_str("v"), 42) == LSM_OK);
    wal_close(wal);
...
static void test_wal_null_args(void) {
    assert(wal_open(NULL, false, NULL) == LSM_ERR_INVALID_ARG);
    assert(wal_sync(NULL) == LSM_ERR_INVALID_ARG);
    assert(wal_recover(NULL, NULL, NULL) == LSM_ERR_INVALID_ARG);
}
