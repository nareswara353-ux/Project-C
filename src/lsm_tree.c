...
LsmStatus lsm_put(LsmTree* tree, LsmSlice key, LsmSlice value) {
    if (!tree || !key.data || !value.data) return LSM_ERR_INVALID_ARG;
    tree->seq++;
    return LSM_OK;
}

LsmStatus lsm_get(LsmTree* tree, LsmSlice key, LsmSlice* out_value) {
    if (!tree || !key.data || !out_value) return LSM_ERR_INVALID_ARG;
    return LSM_ERR_NOT_FOUND;
}

LsmStatus lsm_delete(LsmTree* tree, LsmSlice key) {
    if (!tree || !key.data) return LSM_ERR_INVALID_ARG;
    tree->seq++;
    return LSM_OK;
}

LsmStatus lsm_iterator(LsmTree* tree, LsmIterator** out) {
    if (!tree || !out) return LSM_ERR_INVALID_ARG;
    *out = NULL;
    return LSM_OK;
}
