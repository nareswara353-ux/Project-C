...
typedef void (*WalRecordHandler)(uint8_t op, LsmSlice key, LsmSlice value, uint64_t seq, void* user);

static LsmStatus wal_reader_read_all(struct WalReader* r, WalRecordHandler handler, void* user) {
    if (!r || !handler) return LSM_ERR_INVALID_ARG;
    if (lseek(r->fd, 0, SEEK_SET) < 0) return LSM_ERR_IO;
    uint8_t op;
    uint64_t seq;
    ...
    while ((n = read(r->fd, &op, 1)) == 1) {
        if (read(r->fd, &seq, sizeof(seq)) != sizeof(seq)) break;
        if (read(r->fd, &key_len, sizeof(key_len)) != sizeof(key_len)) break;
        if (key_len > sizeof(key_buf)) break;
        if (read(r->fd, key_buf, key_len) != (ssize_t)key_len) break;
        if (read(r->fd, &val_len, sizeof(val_len)) != sizeof(val_len)) break;
        if (val_len > sizeof(val_buf)) break;
        if (read(r->fd, val_buf, val_len) != (ssize_t)val_len) break;
...
LsmStatus wal_replay_into_memtable(const char* wal_path, void* memtable) {
    if (!wal_path || !memtable) return LSM_ERR_INVALID_ARG;
    struct WalReader* r = NULL;
    LsmStatus st = wal_reader_open(wal_path, &r);
    if (st != LSM_OK) return st;
