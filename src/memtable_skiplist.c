...
LsmStatus skiplist_insert(SkipList *list, LsmInternalKey key, LsmSlice value) {
    ...
    for (int i = list->max_level; i >= 0; i--) {
        while (cur->next[i] &&
               list->cmp(cur->next[i]->key.user_key.data, cur->next[i]->key.user_key.len,
    ...
    cur = cur->next[0];
    if (cur && list->cmp(cur->key.user_key.data, cur->key.user_key.len, key.user_key.data,
                         key.user_key.len) == 0) {
        cur->value = value;
        return LSM_OK;
    }
