...
static LsmStatus wal_append_record(Wal* wal, uint8_t op, LsmSlice key, LsmSlice value, SequenceNumber seq) {
    if (!wal || !key.data) return LSM_ERR_INVALID_ARG;
    ...
    if (written < 0) return LSM_ERR_IO;
    wal->offset += written;
    if (wal->sync) {
        if (fsync(wal->fd) < 0) return LSM_ERR_IO;
    }
    return LSM_OK;
}
...
LsmStatus wal_sync(Wal* wal) {
    if (!wal) return LSM_ERR_INVALID_ARG;
    if (fsync(wal->fd) < 0) return LSM_ERR_IO;
    return LSM_OK;
}
...
typedef void (*WalRecoverCallback)(uint8_t op, LsmSlice key, LsmSlice value, SequenceNumber seq, void* user);

LsmStatus wal_recover(Wal* wal, WalRecoverCallback cb, void* user) {
    if (!wal || !cb) return LSM_ERR_INVALID_ARG;
    if (lseek(wal->fd, 0, SEEK_SET) < 0) return LSM_ERR_IO;
    uint8_t op;
    uint64_t seq;
    ...
    while ((n = read(wal->fd, &op, 1)) == 1) {
        if (read(wal->fd, &seq, sizeof(seq)) != sizeof(seq)) break;
        if (read(wal->fd, &key_len, sizeof(key_len)) != sizeof(key_len)) break;
        if (key_len > sizeof(key_buf)) break;
        if (read(wal->fd, key_buf, key_len) != (ssize_t)key_len) break;
        if (read(wal->fd, &val_len, sizeof(val_len)) != sizeof(val_len)) break;
        if (val_len > sizeof(val_buf)) break;
        if (read(wal->fd, val_buf, val_len) != (ssize_t)val_len) break;
